...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_item') {
        ...
        // Check phiếu chưa hoàn thành
        $r = $conn->query("SELECT trang_thai FROM phieu_nhap WHERE ma_phieu=$ma_phieu");
        $p = $r->fetch_assoc();
        if ($p['trang_thai']) { setFlash('error','Không thể sửa phiếu đã hoàn thành.'); }
        else {
            if ($sl <= 0 || $gia <= 0) { setFlash('error','Số lượng và giá nhập phải lớn hơn 0.'); }
            else {
    ...
    if ($action === 'complete') {
        ...
        $p = $r->fetch_assoc();
        if ($p['trang_thai']) { setFlash('error','Phiếu đã hoàn thành rồi.'); }
        else {
            $items = $conn->query("SELECT * FROM chitiet_nhap WHERE ma_phieu=$ma_phieu");
            if (!$items->num_rows) { setFlash('error','Phiếu không có sản phẩm nào.'); }
            else {
                ...
                try {
                    ...
                    $conn->commit();
                    setFlash('success','Hoàn thành phiếu nhập! Tồn kho đã được cập nhật theo phương pháp bình quân.');
                } catch(Exception $e) {
                    $conn->rollback();
    ...
    if ($action === 'delete_phieu') {
        ...
        $r = $conn->query("SELECT trang_thai FROM phieu_nhap WHERE ma_phieu=$ma_phieu");
        $p = $r->fetch_assoc();
        if ($p['trang_thai']) { setFlash('error','Không thể xóa phiếu đã hoàn thành.'); }
        else {
...
$phieus = $conn->query("SELECT pn.*, (SELECT COUNT(*) FROM chitiet_nhap WHERE ma_phieu=pn.ma_phieu) as so_sp FROM phieu_nhap pn WHERE $where ORDER BY pn.ma_phieu DESC");
...
<div class="d-flex justify-between align-center mb-20">
    <div><h2 class="page-title">Quản Lý Nhập Hàng</h2><p class="page-sub">Phiếu nhập hàng & cập nhật tồn kho bình quân</p></div>
    <?php if (!$edit_phieu): ?>
...
<div class="admin-card mb-20">
    <div class="admin-card-header">
        <h3>📦 Phiếu Nhập #<?= $edit_phieu['ma_phieu'] ?> — <?= date('d/m/Y', strtotime($edit_phieu['ngay_nhap'])) ?>
            <?= $edit_phieu['trang_thai'] ? '<span class="badge badge-success ml-8">Đã hoàn thành</span>' : '<span class="badge badge-warning">Chưa hoàn thành</span>' ?>
        </h3>
    ...
    <div class="admin-card-body">
        ...
        <form method="POST" class="d-flex gap-8 align-center flex-wrap mb-20" style="background:var(--gray-light);padding:16px;border-radius:6px;">
            ...
            <div class="filter-item" style="min-width:120px;">
                ...
                <input type="number" name="so_luong" class="form-control sl-input" min="1" value="1" required>
            </div>
            <div class="filter-item" style="min-width:160px;">
                <label>Giá nhập (₫/đvt)</label>
                <input type="number" name="gia_nhap" class="form-control gia-input" min="0" step="1000" value="0" required>
            </div>
            <div style="padding-top:20px;">
                <button type="submit" class="btn btn-success">+ Thêm Vào Phiếu</button>
            </div>
        </form>
        <?php endif; ?>
        ...
        <table class="data-table">
            <thead><tr><th>Sản phẩm</th><th>Loại</th><th>SL nhập</th><th>Giá nhập</th><th>Thành tiền</th><?= !$edit_phieu['trang_thai'] ? '<th></th>' : '' ?></tr></thead>
            <tbody>
                <?php
                $tong_phieu = 0;
                ...
                <tr class="import-row">
                    ...
                    <td><span class="sl-input"><?= $it['so_luong'] ?></span></td>
                    <td><?= number_format($it['gia_nhap'],0,',','.') ?> ₫</td>
                    <td class="fw-600 tt-cell"><?= number_format($tt,0,',','.') ?> ₫</td>
                    <?php if (!$edit_phieu['trang_thai']): ?>
                    <td>
                ...
                <tr><td colspan="4" style="text-align:right;font-weight:600;">Tổng cộng:</td>
                    <td colspan="2" id="importTotal" class="fw-600 text-walnut"><?= number_format($tong_phieu,0,',','.') ?> ₫</td></tr>
            </tbody>
        </table>
        ...
        <div class="d-flex gap-8 mt-16">
            <form method="POST">
                ...
                <input type="hidden" name="ma_phieu" value="<?= $edit_phieu['ma_phieu'] ?>">
                <button type="submit" class="btn btn-success confirm-delete">✓ Hoàn Thành Phiếu & Cập Nhật Tồn Kho</button>
            </form>
            <form method="POST">
                <input type="hidden" name="action" value="delete_phieu">
                <input type="hidden" name="ma_phieu" value="<?= $edit_phieu['ma_phieu'] ?>">
                <button type="submit" class="btn btn-danger confirm-delete">Hủy Phiếu</button>
            </form>
        </div>
        <p class="form-hint mt-8">⚠️ Sau khi hoàn thành, tồn kho sẽ được cập nhật theo giá bình quân và không thể sửa phiếu.</p>
        <?php endif; ?>
    </div>
</div>
<?php endif; ?>
...
<form method="GET" class="admin-filter">
    ...
    <a href="<?= SITE_URL ?>/admin/imports.php" class="btn btn-secondary btn-sm">Reset</a>
</form>

<div class="admin-card">
    <div style="overflow-x:auto;">
        <table class="data-table">
            <thead><tr><th>#</th><th>Ngày nhập</th><th>Số SP</th><th>Ghi chú</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
                <?php if (!$phieus->num_rows): ?>
                <tr class="empty-row"><td colspan="6">Chưa có phiếu nhập nào</td></tr>
                ...
                <tr>
                    ...
                    <td><span class="badge <?= $p['trang_thai'] ? 'badge-success' : 'badge-warning' ?>"><?= $p['trang_thai'] ? 'Đã hoàn thành' : 'Đang soạn' ?></span></td>
                    <td>
