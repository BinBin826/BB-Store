...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if (!$ten || !$loai) { setFlash('error','Vui lòng điền đầy đủ thông tin.'); redirect(SITE_URL.'/admin/products.php?add=1'); }

        $hinh = handleUpload('hinh');
        if ($hinh === false) { setFlash('error','File ảnh không hợp lệ.'); redirect(SITE_URL.'/admin/products.php?add=1'); }
    ...
    if ($action === 'edit') {
        ...
        if ($new_hinh) {
            ...
            $h = $conn->real_escape_string($new_hinh);
            $hinh_sql = ",hinh='$h'";
        } elseif ($del_img) {
            if ($cur['hinh'] && file_exists(UPLOAD_PATH . $cur['hinh'])) @unlink(UPLOAD_PATH . $cur['hinh']);
            $hinh_sql = ",hinh=''";
    ...
    if ($action === 'delete') {
        ...
        $r  = $conn->query("SELECT hinh FROM san_pham WHERE ma_sp=$id");
        $sp = $r->fetch_assoc();
        // Nếu chưa nhập hàng -> xóa hẳn, ngược lại -> ẩn
        ...
        if ($has_import > 0) {
            ...
            setFlash('warning','Sản phẩm đã được ẩn (đã có lịch sử nhập hàng).');
        } else {
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="tl_loi_nhuan" class="form-control" value="<?= $edit['tl_gb'] ?? 20 ?>" min="0" max="500" step="0.5">
                </div>
                <div class="form-group">
                    <label class="form-label">Trạng thái hiển thị</label>
                    <label class="form-check" style="padding-top:10px;">
                        <input type="checkbox" name="trang_thai" value="1" <?= (!$edit || $edit['trang_thai'])?'checked':'' ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="so_luong_ban_dau" class="form-control" value="0" min="0">
                </div>
                <div class="form-group">
                    <label class="form-label">Giá nhập ban đầu (₫)</label>
                    <input type="number" name="gia_ban_dau" class="form-control" value="0" min="0">
                </div>
                <?php endif; ?>
                <div class="form-group full">
                ...
                <div class="form-group full">
                    ...
                    <div style="margin-bottom:8px;display:flex;align-items:center;gap:12px;">
                        <img src="<?= UPLOAD_URL . $edit['hinh'] ?>" class="current-img" alt="">
                        <label class="form-check">
                            <input type="checkbox" name="delete_image" value="1"> Xóa hình hiện tại
                        </label>
                    </div>
                    <?php endif; ?>
                    <input type="file" name="hinh" class="form-control" accept="image/*" data-preview="imgPreview">
                    <img id="imgPreview" class="img-preview" src="" alt="Preview">
                    <span class="form-hint">Định dạng: JPG, PNG, WebP. Tối đa 2MB.</span>
                </div>
            </div>
            <div class="d-flex gap-8 mt-12">
                <button type="submit" class="btn btn-primary"><?= $edit ? 'Cập Nhật' : 'Thêm Sản Phẩm' ?></button>
                <a href="<?= SITE_URL ?>/admin/products.php" class="btn btn-secondary">Hủy</a>
            </div>
        </form>
    </div>
</div>
...
<form method="GET" class="admin-filter">
    ...
    <a href="<?= SITE_URL ?>/admin/products.php" class="btn btn-secondary btn-sm">Reset</a>
</form>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        ...
                        <img src="<?= UPLOAD_URL.$sp['hinh'] ?>" class="product-thumb" alt="">
                        <?php else: ?><div class="no-thumb">🛋️</div><?php endif; ?>
                    </td>
                    <td class="text-muted">#<?= $sp['ma_sp'] ?></td>
                    <td>
                        <strong><?= htmlspecialchars($sp['ten_sp']) ?></strong>
                        <div style="font-size:0.75rem;color:var(--gray-mid);"><?= htmlspecialchars(mb_strimwidth($sp['mo_ta']??'',0,40,'...','UTF-8')) ?></div>
                    </td>
                    <td><?= htmlspecialchars($sp['ten_loai']) ?></td>
                    <td><strong><?= number_format($gia,0,',','.') ?> ₫</strong></td>
                    <td>
                        <?php if ($ton <= 0): ?><span class="out-stock">Hết hàng</span>
                        <?php elseif($ton<=5): ?><span class="low-stock">⚠ <?= $ton ?></span>
                        <?php else: ?><span class="in-stock"><?= $ton ?></span><?php endif; ?>
                    </td>
                    <td>
                        <span class="badge <?= $sp['trang_thai'] ? 'badge-success' : 'badge-secondary' ?>">
                            <?= $sp['trang_thai'] ? 'Đang bán' : 'Đã ẩn' ?>
                        </span>
                    </td>
                    <td>
                        <div class="action-group">
                            ...
                            <form method="POST" style="display:inline;">
                                ...
                                <button type="submit" class="btn btn-xs btn-danger confirm-delete">Xóa/Ẩn</button>
                            </form>
                        </div>
                    </td>
                </tr>
